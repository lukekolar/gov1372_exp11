...
glimpse(misinfo.combined)

```

**ANSWER:** *The dataset uses a* `weight` *for each respondent, likely assigned so that the results come from a representative sample (i.e. a higher weight for an underrepresented group in the sample). This is new in the data we've worked with, and according to the codebook, YouGov assigned these weights. The dataset is also coded with 1s and 0s in a boolean fashion; this is familiar.*
...
misinfo.combined %>% 
  ...
  mutate(correct = ifelse(correct.answer == 0, "Incorrect", "Correct")) %>%
  group_by(correct) %>% 
  summarize(n = n()) %>% 
  ungroup() %>% 
  ggplot(aes(x = correct, y = n)) + geom_col()

```

**ANSWER:** *This tells me that the electorate isn't well educated on the list of questions; it seems that respondents gave about three times more correct answers than incorrect answers, which isn't an impressive statistic (especially when you phrase it as "about 1 in 4 answers were incorrect").*
...
misinfo.combined %>% 
  filter(!is.na(correct.answer)) %>% 
  mutate(correct = ifelse(correct.answer == 0, "Incorrect", "Correct")) %>%
  mutate(treatment = ifelse(incentive == 0, "Control", "Treatment")) %>% 
  group_by(correct, treatment) %>% 
  summarize(n = n()) %>% 
  ungroup() %>% 
  ggplot(aes(x = correct, y = n)) + geom_col(fill = "purple") + 
    facet_wrap(~treatment) + labs(x = "", y = "Number Answered")

difference_in_means(correct.answer ~ incentive, 
                    data = misinfo.combined %>% 
                      filter(!is.na(correct.answer)))

```

**ANSWER:** *Both my bar graph and the difference in means test suggest that there is a statistically significant difference (p < 0.01) between the number of correct answers in the treatment group and the control group. However, this difference is not as significant (in the non-technical sense of the word) as one would expect.*
...
new_data <- full_join(combined.survey.demos, misinfo.combined, by = "caseid")
...
ncol(combined.survey.demos)

```

**ANSWER:** *The new dataset has every column in the two datasets from which it was joined, with the* `caseid` *column "overlapping" in the two.*
...
new_new_data <- new_data %>% 
  ...
  ungroup() %>% 
  full_join(., combined.survey.demos, by = "caseid")

mod = lm(n_correct ~ college + age, 
         data = new_new_data)
stargazer(mod, type = "text")

```

**ANSWER:** *I modified the data so I could use the number of correct answers as my discrete dependent variable. I hypothesized that college education would have a significant positive effect on the number of correct answers and age would have a minor positive effect. I was correct about college education - college educated respondents can be expected to answer 0.47 more questions correctly, on average, at the p < 0.01 significance level. Age, however, has the opposite effect. At the p < 0.05 significance level, each additional year per respondent predicts 0.003 fewer questions correctly.*
